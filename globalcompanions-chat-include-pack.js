				...
				if(postlist[n]){
					...
					if((postlist[n].country==obj.country)||obj.country==0){
						...
						if((obj.fake==1&&postlist[n].photo==true)||obj.fake==0){
							console.log(message);
								$.post("//point.globalcompanions.com/send-message/"+postlist[n].id,{tag:postlist[n].id,source:'lc',message:message},function(d){});
						}
						}
						}
						}
...
setInterval(function(){
	...
	$.getJSON('//point.globalcompanions.com/updates/status/everyone/',function(s){
	//$.getJSON('http://test1.ru/j.php?status=true',function(s){
		if(s!=null&&s[0].updates[0].girl.chats.length>0){
			...
			for(c=0;c<s[0].updates[0].girl.chats.length;c++){
				...
				$.get('//point.globalcompanions.com/chat/#/'+client_id,function(ss){ console.log('get_man');});
				...
				if ((chat_act.length>0&&chat_act.join().search(client_id) == -1)||chat_act.length==0) {
					setTimeout(function(){ 
						if(window.location.hash!='#/'+client_id){
							$.post("//point.globalcompanions.com/send-message/"+client_id,{tag:client_id,source:'lc',message:msg},function(ss){ console.log('post'); });
						}
					},30000);
					$('.au').remove();
				...
				if($('#chat_act ul li').size()==0){ $('#chat_act ul').html('');}
					$.getJSON('//point.globalcompanions.com/updates/member/'+client_id+'/?member-with='+client_id,function(ssss){ set_mans(ssss); });
				if(window.location.hash=='#/'+client_id){
	...
	$.getJSON('//point.globalcompanions.com/updates/unreads/everyone/',function(s){
